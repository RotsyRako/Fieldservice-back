 ...
 */
export class RecognizeImageGeminiRepository {
  ...
  constructor() {
    ...
    try {
      ...
      throw new Error(
        `Impossible d'initialiser Gemini Pro Vision API\n` +
...
Si aucun objet principal n'est identifié, utilise null pour "detectedObject".`;
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          throw new Error(
            `Réponse de Gemini non valide (format JSON attendu): ${responseText.substring(0, 200)}`
          );
        }
      }

      // Normaliser les labels (s'assurer qu'ils ont tous un score)
      const normalizedLabels = (parsedResponse.labels || []).map((label: any) => ({
