...
exports.recognizeImageGeminiController = exports.RecognizeImageGeminiController = void 0;
const recognizeImageGemini_service_1 = require("../service/remote/recognizeImageGemini.service");
const base_response_utils_1 = require("../utils/base_response.utils");
/**
 * Contrôleur pour la reconnaissance d'image avec Google Gemini Pro Vision API
 * Spécialisé pour la reconnaissance de matériel et d'outils
 */
class RecognizeImageGeminiController {
    constructor() {
        /**
        ...
        this.recognizeImage = async (req, res) => {
            try {
                ...
                // Appeler le service
                const result = await this.recognizeImageGeminiService.recognizeImage(imageBuffer);
                if (!result.success) {
                ...
                console.error("❌ Erreur dans RecognizeImageGeminiController.recognizeImage:", error);
        ...
        this.recognizeImageUpload = async (req, res) => {
            try {
                ...
                const result = await this.recognizeImageGeminiService.recognizeImage(file.buffer);
                if (!result.success) {
                    const statusCode = this.getStatusCodeFromError(result.error);
                    return res.status(statusCode).json((0, base_response_utils_1.fail)(result.message));
                }
                ...
                console.error("❌ Erreur dans RecognizeImageGeminiController.recognizeImageUpload:", error);
                return res.status(500).json((0, base_response_utils_1.fail)("Erreur serveur interne"));
            }
        };
        this.recognizeImageGeminiService = new recognizeImageGemini_service_1.RecognizeImageGeminiService();
    }
    /**
...
exports.RecognizeImageGeminiController = RecognizeImageGeminiController;
exports.recognizeImageGeminiController = new RecognizeImageGeminiController();
